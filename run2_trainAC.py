"""
run2_trainAC.py  --  Train 4-class multitask model on Run2 data (Row A + Row C).

Split
-----
  Row A (a1-a4) : 70 % train / 30 % test-A   (split by position)
  Row C (c1-c4) : 70 % train / 30 % test-C   (split by position)
  Row B (b1-b4) : 100 % external test-B      (replicate)

Train set = Row A 70% + Row C 70%  (merged)

Every *eval_interval* epochs the script
  1. saves a checkpoint
  2. evaluates on test-A, test-B, test-C separately
  3. writes per-sample results, per-well / per-condition breakdown

Usage
-----
    python run2_trainAC.py --config configs/run2_trainAC.yaml
"""
...
# ── local imports ────────────────────────────────────────────────────────────
from datasets.run2_dataset import build_run2_trainAC_datasets
...
def _meta_to_list(meta_batch) -> List[Dict[str, Any]]:
    """Batched metadata (dict-of-lists) -> list-of-dicts."""
...
def plot_eval_curves(eh, d, cnames=None):
    if not eh:
        return
    col = dict(test_a="#E53935", test_b="#1E88E5", test_c="#43A047")
    lab = dict(test_a="Test-A (Row A)", test_b="Test-B (Row B)", test_c="Test-C (Row C)")

    # ── Figure 1: main metrics (3x3 grid) ────────────────────────────────
    panels = [
        # (metric_key,          y-label,                   scale100)
    ...
    # ── Figure 2: per-class F1 over epochs ────────────────────────────────
    if cnames:
...
def main():
    ...
    ap.add_argument("--config", default="configs/run2_trainAC.yaml")
    args = ap.parse_args()
    cfg = load_config(args.config)

    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    exp = cfg.get("experiment_name", "run2_trainAC")
    ...
    log.info("Building datasets (trainAC: Row A + Row C combined train) ...")
    dsets = build_run2_trainAC_datasets(dcfg, tx)

    for dn in ["train", "test_a", "test_b", "test_c"]:
        ds = dsets[dn]
        ...
        log.info(f"  {dn:8s}: {len(ds):>6,} samples  rows={rows}  wells={wells}  {dist}")
    ...
    test_dls = {tn: DataLoader(dsets[tn], batch_size=ebs, shuffle=False,
                               num_workers=nw, pin_memory=True)
                for tn in ["test_a", "test_b", "test_c"]}
    ...
    log.info(f"Train: Row A 70% + Row C 70% (combined)")
    log.info(f"Test sets: test_a (Row-A 30%), test_b (Row B all), test_c (Row-C 30%)")
    log.info(f"{'='*70}\n")

    hist = []
    eh: Dict[str, list] = {k: [] for k in ["test_a", "test_b", "test_c"]}
    t0 = time.time()
    ...
    for tn in ["test_a", "test_b", "test_c"]:
        ...
        log.info(f"  {tn:8s}  Acc={a*100:.1f}%  F1m={f1:.4f}  F1w={f1w:.4f}  "
