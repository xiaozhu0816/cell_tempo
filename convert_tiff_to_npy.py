...
Each TIFF stack (e.g., 96 frames of 2048Ã—2448 uint8) is saved as a single
.npy file with shape [N_frames, H, W]. During training, np.load(mmap_mode='r')
lets us read any frame with a simple array index â€” no TIFF header parsing,
no decompression, no tifffile overhead.

Usage:
    python convert_tiff_to_npy.py --data-dirs data/run1 data/run2 data/run3
...
def main():
    ...
    ap.add_argument(
        ...
        help="One or more directories containing TIFF stacks.",
    )
    args = ap.parse_args()
