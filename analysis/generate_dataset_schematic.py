...
DEFAULT_OUT_DIR = ROOT / "analysis" / "generated"
...
def draw_well_grid(ax):
    ...
    ax.text(5.0, 0.45, "6 Ã— 6 acquisition positions = 36 fields of view", ha="center", va="bottom", fontsize=10)
...
def draw_rgb_panel(ax):
    ...
    ax.text(5.0, 1.0, "Hierarchy:\nrun â†’ well â†’ position â†’ timepoint â†’ sample", ha="center", va="center", fontsize=9.2)


def main():
    parser = argparse.ArgumentParser(description="Generate the dataset schematic used for the row-split setup.")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=DEFAULT_OUT_DIR,
        help="Directory where the schematic will be written.",
    )
