"""
run2_train.py  --  Train 4-class multitask model on Run2 data.

Split
-----
  Row A (a1-a4) : 70 % train / 30 % test-A   (split by position)
  Row B (b1-b4) : 100 % external test-B       (replicate 1)
  Row C (c1-c4) : 100 % external test-C       (replicate 2)
...
Usage
-----
    python run2_train.py --config configs/run2_4class.yaml
"""
...
# ── local imports ────────────────────────────────────────────────────────────
from datasets.run2_dataset import build_run2_datasets
...
def main():
    ...
    ap.add_argument("--config", default="configs/run2_4class.yaml")
    args = ap.parse_args()
    cfg = load_config(args.config)

    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    exp = cfg.get("experiment_name", "run2_4class")
    ...
    dsets = build_run2_datasets(dcfg, tx)
    ...
    for dn in ["train", "test_a", "test_b", "test_c"]:
        ...
        log.info(f"  {dn:8s}: {len(ds):>6,} samples  wells={wells}  {dist}")
    ...
    log.info(f"Test sets: test_a (Row-A hold-out), test_b (Row B), test_c (Row C)")
    log.info(f"{'='*70}\n")
