"""
train_rowsplit.py  --  Main training entry point for the paper experiments.

This script implements the cross-well row-split protocol described in the paper:

- rows A and B are used for training,
- row C is held out for testing,
- data can be aggregated across multiple biological runs,
- temporal and single-frame ablations are controlled through config files.
"""
...
from datasets.plate_dataset import build_row_split_dataset
...
def compute_binary_from_4cls(raw, num_classes=4):
    ...
    Classes 0..num_classes-2 â†’ Infected (0), class num_classes-1 â†’ Mock (1).
...
def plot_per_well_accuracy(raw, cnames, title, edir):
    """Bar chart: accuracy per well, colored by condition, sorted worstâ†’best."""
...
def plot_well_condition_heatmap(raw, cnames, title, edir):
    ...
    Handles both simple wells (c1) and run-prefixed wells (Run1_c1)."""
    meta = raw["meta"]; labels = raw["labels"]; preds = raw["preds"]
    wi = defaultdict(list)
    for i, m in enumerate(meta):
        wi[m.get("well", "?")].append(i)

    # Parse well names: "Run1_c1" â†’ (run="Run1", row="c", col=1)
    # or "c1" â†’ (run="", row="c", col=1)
    parsed = {}
    for w in wi:
...
def plot_per_well_regression(raw, title, edir):
    """Bar chart: regression MAE per well, sorted worstâ†’best."""
...
def plot_eval_curves(eh, d, cnames=None):
    ...
    if cnames:
        ...
        for tn in eh:
            ...
            ax2.set_title(f"Per-class F1 â€” {lab.get(tn, tn)}")
...
def eval_and_save(model, loader, cls_crit, reg_crit, dev, clamp,
    ...
    # â”€â”€ Binary (Infected vs Mock) derived from 4-class predictions â”€â”€
    if len(cnames) > 2:
...
def main():
    ...
    ap.add_argument(
        "--config",
        required=True,
        help="Path to a paper row-split config YAML in configs/rowsplit_*.yaml",
    )
    args = ap.parse_args()
    ...
    if "datasets" not in dcfg or not dcfg.get("datasets"):
        raise ValueError(
            "Paper configs must define data.datasets for the row-split protocol."
        )

    log.info("Building row-split datasets ...")
    ...
    # â”€â”€ save artefacts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
