"""
Plate dataset utilities for the paper's row-split experiments.
...
Paper split strategy:
  - Rows A+B are used for training.
  - Row C is held out for testing.
  - Multiple biological runs can be merged under the same row-split rule.
"""
...
# Pattern matches plate filenames like:
#   ..._s5_a1_p01_t01-93_...
...
@dataclass
class PlateItem:
    """A single TIFF stack from one biological run."""
    path: Path
    well: str           # e.g. "a1"
    row: str            # e.g. "a"
    col: int            # e.g. 1
...
def scan_plate_dir(data_dir, npy_override_dir=None) -> List[PlateItem]:
    """Scan all TIFFs in the plate flat directory.
    ...
    if not data_dir.exists():
        raise FileNotFoundError(f"plate data dir not found: {data_dir}")
    ...
    for p in sorted(data_dir.glob("*.tif*")):
        ...
        if npy_path and npy_path.exists():
            # Read .npy header only (~128 bytes) to get shape â€” no data loaded
    ...
    if not items:
        raise RuntimeError(f"No plate TIFFs found in {data_dir}")
    if use_npy:
...
def split_row_a(
    ...
    seed: int = 42,
) -> Tuple[List[PlateItem], List[PlateItem]]:
    """
    Split Row A items into train/test by position (within each well).
    Each well has ~36 positions â†’ ~25 train, ~11 test.
    """
...
# ---------------------------------------------------------------------------
# Fast .npy frame reader â€” direct seek, no mmap
# ---------------------------------------------------------------------------
...
class PlateDataset(Dataset):
    """PyTorch Dataset for Run1 frame samples."""
    ...
    def _load_frame(self, s: PlateSample) -> np.ndarray:
...
# ---------------------------------------------------------------------------
# Temporal Dataset â€” multi-frame input as pseudo-RGB
# ---------------------------------------------------------------------------
class TemporalPlateDataset(PlateDataset):
    """
...
def build_plate_datasets(
    ...
    """
    # Resolve data directory (Linux â†” Windows)
    plate_dir_str = data_cfg["plate_dir"]
    plate_dir = _resolve_path(plate_dir_str)
    if not plate_dir.exists():
        raise FileNotFoundError(f"plate data dir not found: {plate_dir}")
    ...
    # 1. Scan all TIFFs
    all_items = scan_plate_dir(plate_dir)
    ...
    # 5. Build datasets
    datasets = {
        "train": PlateDataset(train_samples, transform=transforms_dict.get("train"), frames_per_hour=fph),
        "test_a": PlateDataset(test_a_samples, transform=transforms_dict.get("test"), frames_per_hour=fph),
        "test_b": PlateDataset(test_b_samples, transform=transforms_dict.get("test"), frames_per_hour=fph),
        "test_c": PlateDataset(test_c_samples, transform=transforms_dict.get("test"), frames_per_hour=fph),
    }
    return datasets
...
# ---------------------------------------------------------------------------
def build_plate_trainAC_datasets(
    data_cfg: Dict,
    transforms_dict: Dict[str, Optional[Callable]],
) -> Dict[str, "PlateDataset"]:
    """
    ...
    Split strategy:
        - Row A (a1-a4): split by position â†’ 70% train, 30% test-A
        - Row C (c1-c4): split by position â†’ 70% train, 30% test-C
    ...
    # 4. Split Row C by position (70/30) â€” reuse split_row_a with C wells
...
# ---------------------------------------------------------------------------
def build_plate_single_row_datasets(
    data_cfg: Dict,
    transforms_dict: Dict[str, Optional[Callable]],
) -> Dict[str, "PlateDataset"]:
    """
    ...
    Controlled by config keys:
        train_row:    e.g. "c"  â†’ Row C wells for 70/30 split
        external_row: e.g. "b"  â†’ Row B wells for 100% external test
    ...
    """
    plate_dir = _resolve_path(str(data_cfg["plate_dir"]))
    if not plate_dir.exists():
        raise FileNotFoundError(f"plate data dir not found: {plate_dir}")
    ...
    datasets = {
        "train":         PlateDataset(train_samples, transform=transforms_dict.get("train"), frames_per_hour=fph),
        "test_held_out": PlateDataset(test_held_samples, transform=transforms_dict.get("test"), frames_per_hour=fph),
        "test_external": PlateDataset(test_ext_samples, transform=transforms_dict.get("test"), frames_per_hour=fph),
    }
    return datasets


# ---------------------------------------------------------------------------
# Build: Cross-dataset â€” one dataset train/test, another dataset external
# ---------------------------------------------------------------------------
def _split_all_wells(
    ...
    """Split ALL wells by position (70/30) â€” used for cross-dataset setup."""
    rng = random.Random(seed)
    train_items, test_items = [], []
...
def build_cross_dataset(
    ...
    Config must provide:
        train_dir:    path to training dataset (all 12 wells â†’ 70/30 split)
        external_dir: path to external dataset (all 12 wells â†’ 100% test)
    ...
    train_all   = scan_plate_dir(train_dir)
    ext_all     = scan_plate_dir(ext_dir)
    ...
    # 4. Build datasets â€” choose class based on temporal config
    ...
    if temporal_cfg and temporal_cfg.get("enabled", False):
        ...
        def _make_ds(samples, tx):
            return TemporalPlateDataset(samples, transform=tx,
                                       frames_per_hour=fph,
                                       temporal_offsets_hours=offsets)
    else:
        def _make_ds(samples, tx):
            return PlateDataset(samples, transform=tx, frames_per_hour=fph)
...
def build_multi_train_external_dataset(
    ...
    merged_train_items: List[PlateItem] = []
    merged_test_int_items: List[PlateItem] = []

    for i, td in enumerate(train_dirs):
        all_items = scan_plate_dir(td)
        all_items = assign_labels(all_items, plate_layout, class_labels)
        train_items, test_int_items = _split_all_wells(
...
def build_row_split_dataset(
    ...
    all_train_items: List[PlateItem] = []
    all_test_items: List[PlateItem] = []
    ...
    for idx, ds_entry in enumerate(ds_cfgs):
        ...
        items = scan_plate_dir(d, npy_override_dir=local_npy)
